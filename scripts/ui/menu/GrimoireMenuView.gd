...
# Le choix de cible se fait via les cadres héros latéraux.
# ------------------------------------------------------------
...
const GRIMOIRE_WRAPPER_SIZE: Vector2 = Vector2(560, 260)
const SPELL_PANEL_SIZE: Vector2 = Vector2(530, 170)


# ------------------------------------------------------------
# ÉCRAN PRINCIPAL
# Liste les sorts de soin hors combat disponibles pour le groupe.
# ------------------------------------------------------------
...
static func show_grimoire_screen(owner, feedback_text: String = "") -> void:
	...
	if is_current_scene_in_combat(owner):
		wrapper.add_child(owner.create_empty_message_label("Le grimoire ne peut pas être utilisé pendant un combat."))
	...
	var added_count: int = add_heal_spell_rows(owner, spell_list)

	if added_count <= 0:
		spell_list.add_child(owner.create_empty_message_label("Aucun sort de soin n'est disponible."))

	var hint_label: Label = owner.create_label(
		"Choisis un sort, puis une cible.",
		12,
...
# Prépare le soin et active la sélection sur les panneaux latéraux.
# ------------------------------------------------------------
...
static func show_grimoire_heal_target_screen(
	...
	var controls_label: Label = owner.create_label(
		"Cible : souris ou ZQSD/flèches. A/Entrée : valider. E/Échap : annuler.",
		13,
		TEXT_COLOR
	)
...
# ------------------------------------------------------------
# APPLICATION DU SORT
# Vérifie les conditions, consomme les PM et soigne la cible.
# ------------------------------------------------------------
...
# Sélectionne les soins hors combat sans créer de suivi de quête.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# JOURNAL EXISTANT
# Envoie seulement un message dans le canal déjà présent.
# Aucun suivi de quête n'est créé.
# ------------------------------------------------------------
...
# Lit les données sans dépendre d'une classe concrète de héros ou de sort.
# ------------------------------------------------------------
