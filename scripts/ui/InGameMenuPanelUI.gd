...
func show_inventory_screen() -> void:
	set_menu_chrome_visible(true)
	clear_content()
	title_label.text = "INVENTAIRE"
	message_label.text = ""

	var text_label: Label = create_label(
		"L'inventaire sera ajouté plus tard.\n\nCe panneau servira à choisir un héros, consulter ses objets, utiliser des potions et gérer l'équipement.",
		16,
...
func create_status_hero_frame(hero, index: int) -> Panel:
	...
	var sub_label: Label = create_label(
		sub_text,
		12,
		Color(0.85, 0.78, 0.64)
	)
	...
	var portrait_label: Label = create_label(
		"",
		12,
	...
	if hero == null:
		var empty_stats_label: Label = create_label(
			"Aucune donnée.",
			12,
	...
	var hpmp_label: Label = create_label(
		"HP " + str(get_int_property(hero, "hp", 0)) + " / " + str(get_int_property(hero, "max_hp", 0))
		+ "   |   MP " + str(get_int_property(hero, "mp", 0)) + " / " + str(get_int_property(hero, "max_mp", 0)),
		12,
		Color(0.82, 0.76, 0.62)
	)
	...
	var stats_label: Label = create_label(
		stats_text,
		12,
		Color(0.82, 0.76, 0.62)
	)
