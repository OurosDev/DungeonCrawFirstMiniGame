...
func show_shop_screen(feedback_text: String = "") -> void:
	...
	var shop_wrapper: VBoxContainer = create_shop_wrapper(Vector2(420, 280), 8)
	content_box.add_child(shop_wrapper)

	shop_wrapper.add_child(create_shop_gold_panel())

	if feedback_text != "":
		shop_wrapper.add_child(create_shop_feedback_label(feedback_text))

	var buy_button: Button = create_menu_button("Acheter")
	buy_button.pressed.connect(show_shop_buy_screen)
	shop_wrapper.add_child(buy_button)

	var sell_button: Button = create_menu_button("Vendre")
	sell_button.pressed.connect(show_shop_sell_screen)
	shop_wrapper.add_child(sell_button)

	var back_button: Button = create_menu_button("Retour menu")
	back_button.pressed.connect(show_main_screen)
...
func show_shop_buy_screen(feedback_text: String = "") -> void:
	...
	var shop_wrapper: VBoxContainer = create_shop_wrapper(Vector2(600, 380), 6)
	content_box.add_child(shop_wrapper)
	...
	var buyable_count: int = add_shop_buy_rows(shop_list)

	if buyable_count <= 0:
		shop_list.add_child(create_empty_message_label("Aucun objet à acheter."))

	shop_wrapper.add_child(create_shop_back_panel("Retour boutique", show_shop_screen))

	var hint_label: Label = create_label("Cliquez sur un objet pour en acheter 1. Échap : retour au jeu", 12, Color(0.70, 0.62, 0.48))
	hint_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	hint_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	shop_wrapper.add_child(hint_label)
...
func create_shop_gold_panel() -> Panel:
	...
	var gold_label: Label = create_label("Or : " + str(GameSession.get_gold()), 15, Color(1.0, 0.82, 0.35))
	gold_label.custom_minimum_size = Vector2(120, 22)
	gold_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	gold_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	gold_center.add_child(gold_label)
...
func create_shop_back_panel(button_text: String, callback: Callable) -> Panel:
	...
	var back_button: Button = create_compact_menu_button(button_text)
...
func add_shop_buy_rows(shop_list: VBoxContainer) -> int:
	...
	for item_id in item_ids:
		...
		shop_list.add_child(create_shop_buy_button(item_id))
		buyable_count += 1
...
func create_shop_buy_button(item_id: String) -> Button:
	...
	button.pressed.connect(on_shop_buy_pressed.bind(item_id))
...
func on_shop_buy_pressed(item_id: String) -> void:
	...
	show_shop_buy_screen(feedback_text)
...
func on_shop_sell_pressed(item_id: String) -> void:
	...
	var feedback_text: String = str(result.get("message", ""))
	show_shop_sell_screen(feedback_text)
