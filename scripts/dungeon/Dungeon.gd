...
func create_current_floor_state() -> Dictionary:
	return DungeonFloorStateHelperScript.create_floor_state(layout, discovered_map_cells)
...
func apply_floor_state(floor_state: Dictionary) -> void:
	layout = DungeonFloorStateHelperScript.apply_floor_state_to_layout(layout, floor_state)
	if floor_state.has("discovered_map_cells"):
		discovered_map_cells = DungeonFloorStateHelperScript.restore_discovered_map_cells_from_state(floor_state)


func serialize_discovered_map_cells(cells: Dictionary) -> Array:
	return DungeonFloorStateHelperScript.serialize_discovered_map_cells(cells)


func restore_discovered_map_cells(serialized_cells) -> void:
	discovered_map_cells = DungeonFloorStateHelperScript.restore_discovered_map_cells(serialized_cells)
...
func discover_around_player() -> void:
	if player == null:
		return
	DungeonAutoMapHelperScript.discover_around_cell(
		discovered_map_cells,
		layout,
		player.grid_cell,
		CLOSED_DOOR_TILE,
...
func discover_visible_line(
	...
	DungeonAutoMapHelperScript.discover_visible_line(
		discovered_map_cells,
		layout,
		origin,
		direction,
...
func discover_cell(cell: Vector2i) -> void:
	DungeonAutoMapHelperScript.discover_cell(discovered_map_cells, layout, cell)
...
func set_layout_tile(cell: Vector2i, new_tile: String) -> void:
	layout = DungeonMapHelperScript.set_layout_tile(layout, cell, new_tile)
...
func is_walkable(cell: Vector2i) -> bool:
	return DungeonMapHelperScript.is_walkable(layout, cell)


func is_inside_map(cell: Vector2i) -> bool:
	return DungeonMapHelperScript.is_inside_map(layout, cell)


func get_layout_tile(cell: Vector2i) -> String:
	return DungeonMapHelperScript.get_layout_tile(layout, cell)
